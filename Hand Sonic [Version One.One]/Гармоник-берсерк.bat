...
:start
curl --config config.txt -d "weaponQuality=0&battleRoundId=%battleRoundId%&side=defender&value=Berserk" https://zuna.e-sim.org/fight.html --output "otvet.txt" --header "Cookie: JSESSIONID=%cookie%"
cls
...
if errorlevel 1 (
	...
	if errorlevel 1 (
		...
		if errorlevel 1 (
			...
			if errorlevel 1 (
			echo Поле битвы не найдено\ошибка авторизации\критическая ошибка
